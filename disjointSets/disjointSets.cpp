...
void load_json(const std::string& file_name, std::vector<int>& dst_list) {
    std::ifstream file("../../../../files/" + file_name);
    if (!file.is_open()) {
...
void load_data(const std::string& csv_file_name, std::map<int, std::vector<int>>& src_to_dsts) {
    std::ifstream file("../../../../files/" + csv_file_name);
    if (!file.is_open()) {
...
void load_data_filtered(const std::string& csv_file_name, const std::vector<int>& dst_list, std::map<int, std::vector<int>>& src_to_dsts) {
    std::ifstream file("../../../../files/" + csv_file_name);
    if (!file.is_open()) {
        std::cerr << "Failed to open file: " << csv_file_name << std::endl;
        return;
...
void group_data(const std::map<int, std::vector<int>>& src_to_dsts, std::map<int, std::vector<int>>& group_to_srcs, std::map<int, std::vector<int>>& group_to_dsts) {
...
int main() {
    const std::string csv_file_name = "src_dst_2.csv";
    const std::string json_file_name = "dest_list_2.json";

    std::vector<int> dst_list; // список запрашиваемых исходящих файлов
    std::map<int, std::vector<int>> src_to_dsts; // SRC_ID ко множеству DST_ID
    std::map<int, std::vector<int>> group_to_srcs; // GROUP_ID ко множеству SRC_ID
    std::map<int, std::vector<int>> group_to_dsts; // GROUP_ID ко множеству DST_ID

    load_json(json_file_name, dst_list);

    // load_data_(csv_file_name, src_to_dsts);
